 ...
 *  * @author CorentinR
 *  
 *  Représente les joueurs humains avec nom, plateau, etc. 
 *  Méthodes permettant de tirer, actualiser, afficher les infos connues, annoncer le résultat d'un tir, etc.
 *  
 */
public class JoueurHumain implements Joueur {
	...
	private Case[][] plateau;    			//plateau du joueur, mis à jour avec les coups joués par l'adversaire
	private Case[][] plateauAdv;			//plateau de l'adversaire, inconnu, mis à jour avec les coups joués par le joueur
	 ...
	 * @param nX entier, taille du plateau, carré
	 */
	...
	/**
	 * Parcours les bateaux de la flotte et les place un à un sur le plateau (méthode Bateau.placer)
	 */
	public void initialiser (Bateau[] flotte, int paramAuto, int paramCpu)
	{
		...
		nombreBatAdv = flotte.length;    //initialise nombreBatAdv avec le nombre réel de bateau dans la flotte
	...
	/**
	 * Affiche le dernier coup de l'adversaire avant de jouer, en informant des coordonnées,
	 * du resultat du tir et du nom du bateau touché/coulé si il y a lieu + affiche les plateaux mis à jour.
	 * 
	 * @param etatTir Etat du tir renvoyé par l'adversaire (MANQUE, TOUCHE, COULE)
	 * @param coordTir coordonnées du tir de l'adversaire
	 */
	public void recap(Etat etatTir, Coord coordTir)
	{
		if(coordTir == null)
			System.out.print("Vous êtes le premier à jouer, c'est parti !\n\n");
		else
		{
			System.out.print("Votre adversaire a tiré en " + Jeu.convertNL(coordTir.getX()) + "-" + coordTir.getY() + " et ");
			switch(etatTir)
			{
				case MANQUE : System.out.println("n'a rien touché, ouf!");break;
				case TOUCHE : System.out.println("a touché votre " + (plateau[coordTir.getX()][coordTir.getY()].getBat()).getNom() + "!");break;
				case COULE : System.out.println("a coulé votre " + (plateau[coordTir.getX()][coordTir.getY()].getBat()).getNom() + "! Aïe...");break;
	...
	/**
	 * Invite le joueur à tirer. Demande des coordonnées valides et les renvoit dans un Coord seulement si elle n'ont pas
	 * déjà été jouées. Boucle sinon.
	 * 
	...
	public Coord tirer()
	{
		...
		do
		{
			...
			if (plateauAdv[x][y].getDeja()==true)					//test si le coup n'a pas deja été joué, boucle sinon
			{
	...
	/**
	 *  Reçoit les coordonnées du tir adverse et renvoit s'il y a un bateau ou non.
	 *  ("accesseur" évitant d'accéder directement aux champs de l'autre joueurs)
	 *  
	 *  @param coordTir coordonnées du tir adverse (return de adv.tirer)
	 *  
	 *  @return Etat BATEAU ou EAU de la case visée par l'adversaire (param de adv.resultatTir)
	 */
	...
	/**
	 * Informe le joueur du résultat de son tir, teste si le bateau est coulé (appelle adv.testCouler), et actualise son plateauAdv
	 * et le plateau de l'adversaire (appelle adv.refresh)
	 * 
	 * @param adv Joueur adverse (permet d'appeler adv.refresh)
	 * @param result Etat EAU ou BATEAU de la case visée (return de adv.recevoir)
	 * @param coordTir coordonnées du tir adverse (return de adv.tirer)
	 * 
	...
	/**
	 *  Test si le bateau occupant la case visée a 0 pV (coulé) ou non (seulement touché)
	 *  
	 *  @param coordTir coordonnées de la case visée par l'adversaire (return de adv.tirer)
	 *  
	...
	/**
	 *  Actualise le plateau de l'adversaire en meme temps que celui du joueur (appelle par adv.resultatTir) et renvoit, si touché, l'identifiant du bateau touché
	 *  
	 *  @param result Etat MANQUE, TOUCHE ou COULE du coup de l'adversaire
	 *  @param coordTir coordonnées de la case visée par l'adversaire (return de adv.tirer)
	 *  
	 *  @return entier informant, si touché/coulé, de l'identifiant du bateau visé 
	 */
	...
	/**
	 * Test si le joueur a gagné ou non (nombreBatAdv=0), l'informe dans ce cas, et permet de stopper la partie
	 * 
	 * @return booléen true si le joueur a gagné : permet de sortir de la boucle de jeu
	 */
	public boolean victoire()
	{
		if (nombreBatAdv==0)
		{
			System.out.println("C'est gagné ! C'est gagné ! \n°*o Bravo " + name + "!!! o*° \n\n Vous avez gagné en " + Jeu.tour + " tours.");
			return true;
		}
	 ...
	 * 			ET le plateau du joueur avec les coups de l'adversaire, côte à côte
	...
	/**
	 * second "toString" n'affichant que le plateau du joueur, utilisé pendant le placement des bateaux
	 * 
	...
	public String getName()			//permet d'accéder au nom du joueur dans Jeu, pour demander clairement si le joueur nommé est prêt									
	{								//ceci évite les confusions et le dévoilement non volontaire du plateau adverse
		return name;
	}
