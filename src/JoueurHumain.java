...
public class JoueurHumain implements Joueur {
	...
	public JoueurHumain(int nX) {
		...
		plateauAdv = new Case[X][X]; // remplit le plateauAdv de case INC, sans
										// bateau
		for (int i = 0; i < X; i++) {
			for (int j = 0; j < X; j++) {
		...
		System.out.println("\nQuel est votre nom ? :"); // demande au joueur son
														// nom
		name = Jeu.sc.next();
	...
	public void initialiser(Bateau[] flotte, int paramAuto, int paramCpu) {
		for (int i = 0; i < flotte.length; i++) // parcours flotte et appelle
												// Bateau.placer
		{
			if (paramAuto == 1) {
		...
		nombreBatAdv = flotte.length; // initialise nombreBatAdv avec le nombre
										// réel de bateau dans la flotte
	...
	public void recap(Etat etatTir, Coord coordTir) {
		if (coordTir == null)
			...
			case COULE:
				System.out.println(
						"a coulé votre " + (plateau[coordTir.getX()][coordTir.getY()].getBat()).getNom() + "! Aïe...");
				break;
			default:
	...
	public Coord tirer() {
		...
		do {
			...
			if (plateauAdv[x][y].getDeja() == true) // test si le coup n'a pas
													// deja été joué, boucle
													// sinon
			{
	...
	public String getName() // permet d'accéder au nom du joueur dans Jeu, pour
							// demander clairement si le joueur nommé est prêt
